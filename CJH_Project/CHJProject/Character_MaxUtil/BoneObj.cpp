#pragma once
#include "BoneObj.h"
HRESULT BoneObj::CreateVertexData()
{
	HRESULT hr = S_OK;
...
HRESULT BoneObj::CreateIndexData()
{
	HRESULT hr = S_OK;
	return hr;
}
int BoneObj::GetParentIndex(const TCHAR*	strName)
{
...
void BoneObj::GetVIData(HMesh& mesh)
{
	HRESULT hr = S_OK;
	if (mesh.m_TriList.size() <= 0)
	{
		return;
	}
	mesh.m_vbList.reserve(mesh.m_TriList.size() * 3);
	for (int iFace = 0; iFace < mesh.m_TriList.size(); iFace++)
	{
		for (int iVertex = 0; iVertex < 3; iVertex++)
		{

			int iPos = -1;
			for (int i = 0; i < mesh.m_vbList.size(); i++)
			{
				if (mesh.m_vbList[i] == mesh.m_TriList[iFace].v[iVertex])
				{
					iPos = i;
					break;
				}
			...
			if (iPos < 0)
			{
				mesh.m_vbList.push_back(mesh.m_TriList[iFace].v[iVertex]);
				iPos = mesh.m_vbList.size() - 1;
			}
			mesh.m_ibList.push_back(iPos);
		}
	}
}
bool	BoneObj::Load(ID3D11Device* pdvice, ID3D11DeviceContext* pContext, const TCHAR* LoadFile)
{
	...
	for (int iObj = 0; iObj < m_ExpObject.size(); iObj++)
	{
		...
		if (mesh.subMesh.size() > 0)
		{
			for (int i = 0; i < mesh.subMesh.size(); i++)
			{
				...
				mesh.subMesh[i].m_iNumVertex = m_ExpObject[iObj].subMesh[i].m_vbList.size();
				...
				// PNCT 버퍼에 한번에 몰아넣음
				std::copy(m_ExpObject[iObj].subMesh[i].m_vbList.begin(),
					m_ExpObject[iObj].subMesh[i].m_vbList.end(),
					back_inserter(m_VertexData));
		...
		else
		{
			...
			mesh.m_iNumVertex = m_ExpObject[iObj].m_vbList.size();
			...
			// PNCT 버퍼에 한번에 몰아넣음
			std::copy(m_ExpObject[iObj].m_vbList.begin(),
				m_ExpObject[iObj].m_vbList.end(),
				back_inserter(m_VertexData));
...
bool	BoneObj::Parsing(const TCHAR* FileName)
{
	...
	for (int iObj = 0; iObj < m_ExpObject.size(); iObj++)
	{
		...
		{
			_fgetts(m_pBuffer, 256, m_pStream);
			_stscanf(m_pBuffer, _T("\t %d %f %f %f %f %f %f  \n"),
...
bool BoneObj::Release()
{
	return true;
}

bool	BoneObj::OpenStream(const TCHAR* FileName)
{
