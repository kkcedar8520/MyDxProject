...
bool  JH_Screen::Frame()
{

	return true;
}
bool JH_Screen::Render()
{
	JH_ShapePlane::Render();
	m_pScreen->Render();
	return true;
}
void JH_Screen::Process(JH_Model*user)
{
	m_pScreen->Process(user);

}
void JH_Screen::SetTransition(DWORD dwEvent)
{
...
//bool JH_Screen::SetArrayRect(vector<rtList>& DrtList, int width, int height, float fAnimaTime)
//{
//
//
//
