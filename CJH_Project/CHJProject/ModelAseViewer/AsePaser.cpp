...
void  AsePaser::Tokenize(C_STR text, C_STR token, OutputIterator DataList)
{
	auto start = 0;
	auto stop = 0;
...
int AsePaser::SearchTokenArray(vector<string>& secction)
{
	int iRet = -1;
	while (1)
	{
...
int AsePaser::GetData(string FindStr, void* pData, int iType)
{
	int iLine = -1;
	char szTemp[255] = { 0,};
	if ((iLine=FindString(FindStr)) > 0)
	{
...
int AsePaser::FindString(string szFind)
{
...
bool  AsePaser::LoadScene()
{
	...
	if (GetData("SCENE ") > 0)
	{
		...
		GetData("SCENE_TICKSPERFRAME", &m_SCeneInfo.TickSperFrmae, INT_DATA);
	}

	return true;
}
bool AsePaser::LoadMateRial()
{
...
bool AsePaser::LoadMateRial( Mtrl* pMtrl)
{
	int iType;
...
void AsePaser::GetUVData(int iLine,vector<D3DXVECTOR2>&UVList)
{
	char szTemp[256] = { 0 };
...
void AsePaser::GetTFaceData(int iLine, vector<aseINDEX>&UVFacesList)
{
...
void AsePaser::GetFaceNormal(int iLine, vector<FaceNormal>&FaceNormalList)
{
...
bool AsePaser::LoadTexture(Mtrl* pMtrl)
{
