...
void	xAseObj::Convert()
{
	...
	for (int iObj = 0; iObj < m_Parser.m_ObjList.size(); iObj++)
	{
		...
		mesh.m_szName = m_Parser.m_ObjList[iObj]->szName;
		mesh.m_szParentName = m_Parser.m_ObjList[iObj]->szParentName;


		D3DXMATRIX matParentInverse,matDefaultAnim;
		D3DXMatrixIdentity(&matParentInverse);
		mesh.m_iParentIndex = m_Parser.GetParentIndex(mesh.m_szParentName);

		if (mesh.m_iParentIndex>=0)
		{
			D3DXMatrixInverse(&matParentInverse, NULL, &m_Parser.m_ObjList[mesh.m_iParentIndex]->m_matWorld);
		}
...
bool xAseObj::Frame()
{
	...
	for (int iObj = 0; iObj <m_MeshList.size(); iObj++)
	{
		...
		vPos = m_MeshList[iObj].m_vAnimPos;
		Quat = m_MeshList[iObj].m_qAnimRotation;
		vScale = m_MeshList[iObj].m_vAnimScale;
		qScale = m_MeshList[iObj].m_qAnimScaleRotation;

		D3DXMatrixScaling(&matScale, vScale.x, vScale.y, vScale.z);
		D3DXMatrixRotationQuaternion(&matRotation, &m_MeshList[iObj].m_qAnimRotation);
		...
		if (m_MeshList[iObj].m_AnimTrackPos.size()>0)
		{
			...
			start.iTick = 0;
			start.p = vPos;
			int finalindex = m_MeshList[iObj].m_AnimTrackPos.size() - 1;
			end = m_MeshList[iObj].m_AnimTrackPos[finalindex];
		...
		if (m_MeshList[iObj].m_AnimTrackRotate.size() > 0)
		{
			...
			start.iTick=0;
			start.q = Quat;
			int finalindex=m_MeshList[iObj].m_AnimTrackRotate.size()-1;
			end = m_MeshList[iObj].m_AnimTrackRotate[finalindex];
		...
		if (m_MeshList[iObj].m_AnimTrackScale.size() > 0)
		{
			...
			int finalindex = m_MeshList[iObj].m_AnimTrackScale.size() - 1;
			end = m_MeshList[iObj].m_AnimTrackScale[finalindex];
		...
		m_MeshList[iObj].m_matCalculation = matScale* matRotation;
		m_MeshList[iObj].m_matCalculation._41 = vPos.x;
		m_MeshList[iObj].m_matCalculation._42 = vPos.y;
		m_MeshList[iObj].m_matCalculation._43 = vPos.z;

		if (m_MeshList[iObj].m_iParentIndex >= 0)
		{
			D3DXMATRIX matParent = m_MeshList[m_MeshList[iObj].m_iParentIndex].m_matCalculation;
			m_MeshList[iObj].m_matCalculation *= matParent;
		}
	}
...
bool xAseObj::Render()
{
	...
	for (int iObj = 0; iObj <m_MeshList.size(); iObj++)
	{
		...
		m_MeshList[iObj].m_matCalculation *= m_matWorld;
		D3DXMatrixTranspose(&m_MeshList[iObj].m_cbData.matWorld,
			&m_MeshList[iObj].m_matCalculation);
		...
		if (m_MeshList[iObj].subMesh.size() > 0)
		{
			...
			{
				if (m_MeshList[iObj].subMesh[iSub].m_iNumVertex <= 0) continue;
